                    ...
                    <div class="col-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                ...
                                <a href="{{route('admin.jobs.create')}}" class="btn btn-success float-right" title="{{__('Thêm mới công việc')}}">{{__('Thêm mới công việc')}}</a>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <table id="example1" class="table table-bordered dt-responsive" style="table-layout: fixed;width: 100%;">
                                    ...
                                    @foreach($jobs as $key => $job)
                                        <tr>
                                            ...
                                            <td>{{ $job->city->name }}</td>
                                            <td>{!! \App\Helpers\Common::showStatus($job->is_active) !!}</td>
                                            <td>
                                                <button class="btn btn-warning btn-sm btn-update" onclick="updateStatus('{{route('admin.jobs.show-status', $job->id)}}')"
                                                        data-toggle="modal" data-target="#status">
                                                    <i class="fas fa-wrench"></i>
                                                </button>
                                                <a class="btn btn-info btn-sm btn-edit" href="{{route('admin.jobs.edit', $job->id)}}"><i class="fas fa-pencil-alt"></i></a>
